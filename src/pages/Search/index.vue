        ...
        <div class="details clearfix">
          ...
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <!-- 箭头向上 icon-jiantou_xiangshang_o  箭头向下 icon-jiantou_xiangxia_o -->
                <li :class="{active: isOne}" @click="changeOrder(1)">
                  <a>综合<span v-show="isOne" class="iconfont" :class="{'icon-jiantou_xiangshang_o':isAsc,'icon-jiantou_xiangxia_o':isDesc}"></span></a>
                </li>
                <li :class="{active: isTwo}" @click="changeOrder(2)">
                  <a>价格<span v-show="isTwo" class="iconfont" :class="{'icon-jiantou_xiangshang_o':isAsc,'icon-jiantou_xiangxia_o':isDesc}"></span></a>
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      searchParams: {
        ...
        //关键字
        keyword: "",
        //排序，初始的状态应该是综合|降序，1代表综合，2代表价格，asc升序，desc降序
        order: "1:desc",
        //分页器：代表第几页
        pageNo: 1,
        //每一页展示数据个数
  ...
  methods: {
    ...
    changeOrder(flag){
      ...
      if(orginFlag == flag){
        //第一次一定点击的是综合
        newOrder = `${orginFlag}:${orignSort=='desc'?'asc':'desc'}`
      }else{
        //如果点击按钮不同
        newOrder = `${flag}:${'desc'}`
      }
  ...
  watch:{
    $route(newvalue,oldvalue){
       ...
       //再次发请求
       this.getData()
       //每一次请求完毕，应该把相应的1、2、3级分类的id置空，让他接受下一次的相应的1、2、3
       this.searchParams.category1Id = ''
       this.searchParams.category2Id = ''
       this.searchParams.category3Id = ''
